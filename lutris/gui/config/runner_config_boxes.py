...
class OptionBox(BaseRunnerConfigBox):
    ...
    def from_dict(self, option_dict: Optional[Dict[str, Any]]) -> bool:
        ...
        if isinstance(default, bool):
            self._default_entry = Gtk.Switch(active=default)
        else:
            self._default_entry = Gtk.Entry(text=default)
...
class BaseConfigListBox(BaseRunnerConfigBox):
    def __init__(self, data, derived_widget_type):
        super().__init__()
        ...
        action_id = "Remove"
        remove_action = Gtk.Action(name=action_id, label="Remove item from container")
        remove_action.connect("activate", self._on_delete_row)
        self._remove_widget = None

        menu_item = remove_action.create_menu_item()
    ...
    def to_dict(self) -> List[Any]:
        ...
        for list_box_row in self._list_box.get_children():
            hbox = list_box_row.get_children()[0]
            expander = hbox.get_children()[1]
    ...
    def add_widget(self, new_widget) -> Gtk.Expander:
        ...
        new_widget.connect("button-press-event", self._list_box_popup_menu, list_box_row)
    ...
    def _list_box_popup_menu(self, widget, event, list_box_row):
        """Create popup menu that can be remove list box rows"""
        if event.button != Gdk.BUTTON_SECONDARY:
            return

        self._remove_widget = list_box_row
        self._popup_menu.popup_at_pointer(event)

    def _on_delete_row(self, action):
        if self._remove_widget:
            list_box_row = self._remove_widget
            self._remove_widget = None
            self._on_delete(action, list_box_row)
...
class BaseConfigTextBox(BaseRunnerConfigBox):
    ...
    def to_dict(self) -> str:
        ...
        output_string = ""
        if text := self._text_entry.get_text():
            output_string = text
...
class DescriptionBox(BaseConfigTextBox):
    def __init__(self, **kwargs):
        super().__init__(label=_("Description"), tooltip=("Desciption of the runner"), **kwargs)
...
class BaseConfigGridBox(BaseRunnerConfigBox):
    ...
    def _on_cell_edited(self, widget, path, text, field):
