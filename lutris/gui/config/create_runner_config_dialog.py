...
class EditRunnerConfigDialog(GameDialogCommon):
    ...
    def __init__(self, parent=None, edit_mode=RunnerConfigEditMode.CREATE, runner: Optional[Runner] = None):
        ...
        self.create_runner_box = CreateRunnerBox()

        self._runner_name_entry = Gtk.Entry(visible=True)
        self._runner_name_entry.set_tooltip_text(
            _("The prefix used to name the JSON file for the runner in the form of <runner-file-prefix>.json")
        )
        ...
        # Existing runners cannot have their runner name changed as the save location
        if edit_mode == RunnerConfigEditMode.UPDATE:
            if isinstance(runner, ModelRunner):
                ...
                self.create_runner_box.from_dict(runner.to_dict())
        ...
        self.get_content_area().pack_start(self.create_runner_box, True, True, 0)
        self.show_all()
    ...
    def on_save(self, _widget):
        runner_dict = self.create_runner_box.to_dict()
        runner_name = self._runner_name_entry.get_text()
